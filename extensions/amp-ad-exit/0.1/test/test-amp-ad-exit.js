...
describes.realWin('amp-ad-exit', {
  ...
  beforeEach(() => {
    ...
    json.textContent = JSON.stringify(EXIT_CONFIG);
  ...
  it('should ping tracking URLs by making image requests', () => {
    const open = sandbox.stub(win, 'open', () => {
      return {name: 'fakeWin'};
    });
  ...
  it('should replace custom URL variables', () => {
    ...
    expect(sendBeacon)
        .to.have.been.calledWith('http://localhost:8000/tracking?bar=bar', '');
  });
});
